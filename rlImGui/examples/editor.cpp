   ...
   public:
      ...
      void Show() override
      {
         ...
         if(ImGui::Begin("Image Viewer", &Open, ImGuiWindowFlags_NoScrollbar))
         {
            ...
            if(ImGui::BeginChild(
                   "Toolbar", ImVec2(ImGui::GetContentRegionAvail().x, 25)
               ))
            {
               ...
               ImGui::TextUnformatted(TextFormat(
                   "camera target X%f Y%f", Camera.target.x, Camera.target.y
               ));
               ImGui::EndChild();
            }

            rlImGuiImageRect(
                &ViewTexture.texture, (int)size.x, (int)size.y, viewRect
            );
         }
         ImGui::End();
         ImGui::PopStyleVar();
      ...
      void Update() override
      {
         ...
         if(Focused)
         {
            ...
            {
               // only do this tool when the mouse is in the content area of
               // the window
      ...
      void UpdateRenderTexture()
      {
         ...
         DrawTexture(
             ImageTexture, ImageTexture.width / -2, ImageTexture.height / -2,
             WHITE
         );
...
class SceneViewWindow : public DocumentWindow
{
   public:
      ...
      void Update() override
      {
         ...
         DrawPlane(
             Vector3{ 0, 0, 0 }, Vector2{ 50, 50 }, BEIGE
         ); // simple world plane
         float spacing = 4;
         int count     = 5;
...
int main(int argc, char *argv[])
{
   ...
   InitWindow(
       screenWidth, screenHeight, "raylib-Extras [ImGui] example - ImGui Demo"
   );
   SetTargetFPS(144);
   rlImGuiSetup(true);
